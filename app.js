...
async function compressImageForUpload(file, maxW=1280, quality=0.82){
  ...
  return new File([blob], "cheque.jpg", { type: "image/jpeg" });
}
...
async function uploadChequeImageToCloudinary(file){
  ...
  if(!res.ok) throw new Error(j?.error?.message || "فشل رفع الصورة");
...
function bindDashboard(){
  (async ()=>{
    try{
      ...
      toast("تعذر تحميل البيانات", "danger");
    }finally{
      // re-render same view content only is overkill; just fill sections
      // but if we used setMainLoading, restore page
      const main = $("#main");
...
function bindStatement(){
  ...
  (async ()=>{
    try{
      ...
      toast("تعذر تحميل الصندوق", "danger");
    }
  })();
  ...
  async function loadStatement(){
    try{
      ...
      toast("تعذر بناء الكشف", "danger");
    }finally{
      // restore statement view if loading screen replaced it
      const main = $("#main");
      if(!$("#stTable", main)){
        main.innerHTML = statementView();
        bindStatement();
      }
    }
  }
    ...
    return `
      <div class="card" data-id="${escapeHtml(c.id)}">
        ...
        <div class="row" style="gap:10px; flex-wrap:wrap; margin-top:12px">
          <button class="btn small" type="button" class="btnViewImg">${icon("edit")} صورة</button>
  ...
  $("#chSave").onclick = async ()=>{
    ...
    try{
      ...
      if(imgFile){
        ...
        imageUrl = await uploadChequeImageToCloudinary(imgFile);
      }
