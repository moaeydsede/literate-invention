...
async function statementView(cashboxId){
  ...
  return `
    <div class="card">
      ...
      <div class="card__bd">
        <div class="grid2">
          <div class="tile"><div class="k">افتتاحي للفترة</div><div class="v">${fmtMoney(openingForPeriod)}</div></div>
...
async function openChequeModal(){
  ...
  const m = modal(`
    ...
    <div class="card__bd">
      ...
      <div class="field"><label>رابط صورة (اختياري)</label><input class="input" id="c-img" placeholder="https://..." /></div>
  ...
  m.querySelector("#c-save")?.addEventListener("click", async ()=>{
    ...
    const imageUrl = m.querySelector("#c-img").value.trim();
