...
// ---- Date helpers (robust; required by Cheques & Reports) ----
function parseISODate(s){
  if(!s) return null;

  // Firestore Timestamp {seconds, nanoseconds}
  if(typeof s === "object" && s.seconds){
...
async function listCheques({status="pending"}){
  // Load without composite indexes and without relying on dueDate type consistency
  const qy = query(collection(db, "cheques"), orderBy("createdAt","desc"), limit(400));
  const snap = await getDocs(qy);
  let list = snap.docs.map(d=>{
    const data = d.data();
    // normalize dates for safe sorting/display
    const dueDate = toISODate(data.dueDate) || (typeof data.dueDate==="string" ? data.dueDate : "");
    const issueDate = toISODate(data.issueDate) || (typeof data.issueDate==="string" ? data.issueDate : "");
    return { id:d.id, ...data, dueDate, issueDate };
  });
  ...
  function renderTxTable(rows, opening){
    ...
    return `
      ...
      <table class="table">
        ...
        <tbody>
          ${rows.filter(r=>r.status!=="void").map(r=>{
...
function bindCheques(){
  ...
  (async ()=>{
    try{
      ...
      toastOnce(`تعذر تحميل الشيكات: ${errText(e)}`, "danger", "cheques_load");
    }
  })();
...
$$(".btnDelCheque", host).forEach(btn=>{
      btn.onclick = async (e)=>{
        const id = e.target.closest("[data-id]")?.dataset?.id;
        const cheque = list.find(x=>x.id===id);
        if(!cheque) return;
        if(!confirm(`حذف الشيك #${cheque.chequeNo||""}؟ لا يمكن التراجع.`)) return;
        try{
          await deleteCheque(id);
          toast("تم الحذف", "success");
          render();
        }catch(err){
          toast(`تعذر الحذف: ${errText(err)}`, "danger");
        }
      };
    });
      ...
      <div class="card" data-id="${escapeHtml(c.id)}">
        <div class="row between">
          <div class="col">
            ...
            <div class="p">المبلغ: <b>${fmtMoney(c.amount)}</b> • الاستحقاق: ${escapeHtml(c.dueDate||"—")} ${dueBadge(c.dueDate)}</div>
        ...
        <div class="row" style="gap:10px; flex-wrap:wrap; margin-top:12px">
          ...
          ${state.role==="admin" ? `<button class="btn small danger btnDelCheque">${icon("trash")} حذف</button>` : ``}
...
// default route
if(!location.hash) go("#login");function pad2(n){return String(n).padStart(2,"0");}
function toISODate(val){
  // Accept YYYY-MM-DD string or Firestore Timestamp or Date
  if(!val) return "";
