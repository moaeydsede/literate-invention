...
// v8 Enterprise Stable: Runtime error overlay (does not affect syntax)
(function(){
  const show = (title, detail)=>{
    try{
      ...
      box.innerHTML = `
        <div style="max-width:720px;margin:0 auto;background:#0b1220;border:1px solid rgba(255,255,255,.14);border-radius:18px;padding:14px;color:#e6eefc">
          <div style="font-weight:950;font-size:18px;margin-bottom:8px">${title}</div>
          <div style="font-size:12px;opacity:.85;margin-bottom:10px">إذا كان السبب كاش قديم: امسح Site data أو افتح Incognito.</div>
          <pre style="white-space:pre-wrap;direction:ltr;text-align:left;background:rgba(255,255,255,.06);padding:10px;border-radius:14px;max-height:40vh;overflow:auto">${detail}</pre>
          <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:12px">
            <button id="cb-close" style="padding:10px 12px;border-radius:14px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);color:#e6eefc;font-weight:900">إغلاق</button>
            <button id="cb-reload" style="padding:10px 12px;border-radius:14px;border:1px solid rgba(255,255,255,.16);background:rgba(110,231,255,.18);color:#e6eefc;font-weight:950">إعادة تحميل</button>
          </div>
        </div>`;
      root.appendChild(box);
      box.querySelector("#cb-close")?.addEventListener("click", ()=> box.remove());
      box.querySelector("#cb-reload")?.addEventListener("click", ()=> location.reload());
    }catch(_){}
  };
  window.addEventListener("error", (e)=>{
    // runtime errors
    const msg = e?.error?.stack || e?.message || String(e);
    show("حدث خطأ في التطبيق", msg);
  });
...
function setMainLoading(text="جاري التحميل…"){
  const main = $("#main");
  if(!main) return;
  main.innerHTML = `
    <div class="card">
      <div class="card__bd">
        <div class="loading">
          ...
          <div style="margin-inline-start:12px; font-weight:900">${escapeHtml(text)}</div>
        </div>
...
function showFatalLoadingProblem(){
  const root = $("#app");
  if(!root) return;
  root.innerHTML = `
    <div class="splash">
      <div class="brand">
        ...
        <div class="brand__sub">تأكد من الإنترنت/الـ Hosting ثم أعد المحاولة</div>
      </div>
      <div class="card" style="width:min(520px,100%); text-align:right">
        <div class="card__bd">
          <div class="small">لو ظهرت مشكلة Loading غالباً بسبب:</div>
          <ul style="margin:10px 18px; color: var(--muted)">
            <li>ضعف إنترنت / DNS / VPN</li>
            <li>Firebase rules أو Firestore غير مفعّل</li>
            <li>Cloudinary preset ليس Unsigned</li>
          </ul>
          <button class="btn btn--primary" id="btn-reload">إعادة تحميل</button>
        </div>
      </div>
    </div>`;
...
async function renderPage(){  setMainLoading();
...
<div class="chart-wrap">
  <div class="chart-hd">
    <div class="chart-title">Chart: صافي شهري</div>
...
<div class="chart-wrap">
  <div class="chart-hd">
    <div class="chart-title">Chart: صافي الصناديق</div>
    <span class="badge">Net</span>
  </div>
  <canvas class="chart" id="chart-cashboxes"></canvas>
</div>
...
async function computeReports(from, to){
  const cashboxes = await listCashboxes(true);
  const fromISO = from || "";
  const toISO = to || "";

  const perCashbox = [];
  const monthlyMap = new Map(); // YYYY-MM
  const accountMap = new Map(); // account -> {in,out}

  let grandIn=0, grandOut=0;
  for(const cb of cashboxes){
    const txSnap = await getDocs(query(collection(db,"cashboxes", cb.id, "transactions"), orderBy("date","asc"), orderBy("createdAt","asc")));
    ...
    txSnap.forEach(d=>{
      ...
      const ym = (date && date.length>=7) ? date.slice(0,7) : "Unknown";
      const mm = monthlyMap.get(ym) || { in:0, out:0 };
      if(t.kind==="IN") mm.in += amt; else mm.out += amt;
      monthlyMap.set(ym, mm);

      const acc = (t.account||"").trim() || "—";
      const a = accountMap.get(acc) || { in:0, out:0 };
      if(t.kind==="IN") a.in += amt; else a.out += amt;
      accountMap.set(acc, a);
    });
    grandIn += inSum; grandOut += outSum;
    perCashbox.push({
      id: cb.id, name: cb.name||cb.id, inSum, outSum, net: inSum-outSum, count,
      opening: Number(cb.openingBalance||0), balance: Number(cb.balance||0), active: cb.isActive!==false
    });
  }
  ...
  const monthly = Array.from(monthlyMap.entries())
    ...
    .map(([ym,v])=>({ ym, in:v.in, out:v.out, net:v.in-v.out }));

  const accounts = Array.from(accountMap.entries())
    .map(([account,v])=>({ account, in:v.in, out:v.out, net:v.in-v.out }))
    .sort((a,b)=>Math.abs(b.net)-Math.abs(a.net))
    .slice(0, 20);

  return { perCashbox, monthly, accounts, grandIn, grandOut, net: grandIn-grandOut };
}
...
function fmtShort(n){
  ...
  if(abs >= 1_000_000) return (x/1_000_000).toFixed(1).replace(/\.0$/,'') + "M";
  if(abs >= 1_000) return (x/1_000).toFixed(1).replace(/\.0$/,'') + "K";
...
function drawBarChart(canvas, labels, values){
  ...
  const h = (rect.height || 220) * dpr;
  canvas.width = w; canvas.height = h;
  const ctx = canvas.getContext("2d");
  ctx.clearRect(0,0,w,h);

  const pad = 18*dpr;
  const baseY = h - pad*1.6;
  const baseX = pad*1.4;
  const topY = pad*1.1;
  const rightX = w - pad*1.0;
  ...
  for(let i=0;i<values.length;i++){
    ...
    grad.addColorStop(0, v>=0 ? "rgba(110,231,255,.85)" : "rgba(248,113,113,.85)");
    grad.addColorStop(1, v>=0 ? "rgba(167,139,250,.55)" : "rgba(248,113,113,.45)");
    ctx.fillStyle = grad;
    ...
    ctx.fillText(labels[i] || "", x + bw/2, baseY + 14*dpr);
    ctx.font = `${12*dpr}px ui-sans-serif`;
    ctx.fillText(fmtShort(v), x + bw/2, y - 6*dpr);
  }
}

/* Report */
async function reportView(){
  const from = localStorage.getItem("rep_from") || "";
  const to = localStorage.getItem("rep_to") || "";

  const { perCashbox, monthly, accounts, grandIn, grandOut, net } = await computeReports(from, to);
  const rangeLabel = (from||to) ? `${from||"—"} → ${to||"—"}` : "كل الفترة";

  return `
    <div class="card">
      <div class="card__hd">
        <div><div class="title">تقارير (Pro)</div><div class="sub">فلترة بالتاريخ + Excel + PDF/Print</div></div>
        <div class="hstack">
          <button class="btn btn--ghost" id="rep-print">طباعة / PDF</button>
          <button class="btn btn--ghost" id="rep-export">${icon("excel")} CSV</button>
          <button class="btn btn--ghost" id="rep-export-xlsx">${icon("excel")} XLSX</button>
        </div>
      </div>
      <div class="card__bd">
        <div class="row">
          <div class="field"><label>من</label><input class="input" id="rep-from" type="date" value="${escapeHtml(from)}" /></div>
          <div class="field"><label>إلى</label><input class="input" id="rep-to" type="date" value="${escapeHtml(to)}" /></div>
        </div>
        <div class="hstack">
          <button class="btn" id="rep-apply">تطبيق</button>
          <span class="badge">النطاق: ${escapeHtml(rangeLabel)}</span>
          <span class="badge">الصافي: <b>${fmtMoney(net)}</b></span>
        </div>

        <div class="sep"></div>

        <div class="grid2">
          <div class="tile"><div class="k">إجمالي المقبوضات</div><div class="v">${fmtMoney(grandIn)}</div></div>
          <div class="tile"><div class="k">إجمالي المدفوعات</div><div class="v">${fmtMoney(grandOut)}</div></div>
        </div>
...
<div style="overflow:auto">
          ...
          <table class="table">
            <thead>
              <tr>
                <th>الصندوق</th><th>عدد</th><th>مقبوض</th><th>مدفوع</th><th>الصافي</th><th>افتتاحي</th><th>حالي</th><th>الحالة</th>
              </tr>
            </thead>
            <tbody>
              ${perCashbox.map(c=>`
                <tr>
                  <td class="wrap"><a href="#statement/${encodeURIComponent(c.id)}"><b>${escapeHtml(c.name)}</b></a><div class="small">${escapeHtml(c.id)}</div></td>
                  <td>${c.count}</td>
                  <td>${fmtMoney(c.inSum)}</td>
                  <td>${fmtMoney(c.outSum)}</td>
                  <td><b>${fmtMoney(c.net)}</b></td>
                  <td>${fmtMoney(c.opening)}</td>
                  <td>${fmtMoney(c.balance)}</td>
                  <td>${c.active ? `<span class="badge">نشط</span>` : `<span class="badge">موقوف</span>`}</td>
                </tr>
              `).join("")}
            </tbody>
          </table>
        ...
        <div class="card" style="box-shadow:none">
          <div class="card__hd"><div><div class="title">ملخص شهري</div><div class="sub">In / Out / Net</div></div></div>
          <div class="card__bd" style="overflow:auto">
            <table class="table">
              <thead><tr><th>الشهر</th><th>مقبوض</th><th>مدفوع</th><th>الصافي</th></tr></thead>
              <tbody>
                ${monthly.length ? monthly.map(m=>`
                  <tr>
                    <td class="mono">${escapeHtml(m.ym)}</td>
                    <td>${fmtMoney(m.in)}</td>
                    <td>${fmtMoney(m.out)}</td>
                    <td><b>${fmtMoney(m.net)}</b></td>
                  </tr>
                `).join("") : `<tr><td colspan="4"><span class="small">لا توجد بيانات.</span></td></tr>`}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div class="card" style="box-shadow:none">
          <div class="card__hd"><div><div class="title">Top Accounts</div><div class="sub">أكثر الأطراف تأثيراً (حسب الصافي)</div></div></div>
          <div class="card__bd" style="overflow:auto">
            <table class="table">
              <thead><tr><th>الحساب/الطرف</th><th>مقبوض</th><th>مدفوع</th><th>الصافي</th></tr></thead>
              <tbody>
                ${accounts.length ? accounts.map(a=>`
                  <tr>
                    <td class="wrap"><b>${escapeHtml(a.account)}</b></td>
                    <td>${fmtMoney(a.in)}</td>
                    <td>${fmtMoney(a.out)}</td>
                    <td><b>${fmtMoney(a.net)}</b></td>
                  </tr>
                `).join("") : `<tr><td colspan="4"><span class="small">لا توجد بيانات.</span></td></tr>`}
              </tbody>
            </table>
...
function bindReport(){
  $("#rep-apply")?.addEventListener("click", ()=>{
    localStorage.setItem("rep_from", $("#rep-from").value);
    localStorage.setItem("rep_to", $("#rep-to").value);
    render();
  });
  ...
  $("#rep-export-xlsx")?.addEventListener("click", async ()=>{
    ...
    const { perCashbox, monthly, accounts } = await computeReports(from, to);
    await loadSheetJS();

    const wb = window.XLSX.utils.book_new();
    const ws1 = window.XLSX.utils.json_to_sheet(perCashbox.map(c=>({
      Cashbox: c.name, Id: c.id, Count: c.count, Receipts: c.inSum, Payments: c.outSum, Net: c.net,
      Opening: c.opening, Balance: c.balance, Active: c.active ? "Yes":"No"
    })));
    window.XLSX.utils.book_append_sheet(wb, ws1, "Cashboxes");

    const ws2 = window.XLSX.utils.json_to_sheet(monthly.map(m=>({ Month: m.ym, Receipts: m.in, Payments: m.out, Net: m.net })));
    window.XLSX.utils.book_append_sheet(wb, ws2, "Monthly");

    const ws3 = window.XLSX.utils.json_to_sheet(accounts.map(a=>({ Account: a.account, Receipts: a.in, Payments: a.out, Net: a.net })));
    window.XLSX.utils.book_append_sheet(wb, ws3, "TopAccounts");

    const label = `${from||"all"}_to_${to||"all"}`;
    window.XLSX.writeFile(wb, `reports_${label}.xlsx`);
  });

  $("#rep-export")?.addEventListener("click", async ()=>{
    const from = $("#rep-from").value;
    const to = $("#rep-to").value;
    const { perCashbox } = await computeReports(from, to);
    const header = ["Cashbox","Id","Count","Receipts","Payments","Net","Opening","Balance","Active"];
    const lines = [header.join(",")];
    for(const c of perCashbox){
      lines.push([
        (c.name||"").replaceAll(","," "),
        (c.id||"").replaceAll(","," "),
        c.count, c.inSum, c.outSum, c.net, c.opening, c.balance, c.active ? "Yes":"No"
      ].join(","));
    }
    const blob = new Blob([lines.join("
")], { type:"text/csv;charset=utf-8" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `reports_${from||"all"}_to_${to||"all"}.csv`;
    a.click();
    URL.revokeObjectURL(a.href);
  });
  ...
  (async ()=>{
    try{
      const from = $("#rep-from")?.value || "";
      const to = $("#rep-to")?.value || "";
      const { perCashbox, monthly } = await computeReports(from, to);
      const ml = monthly.slice(-12);
      drawBarChart($("#chart-monthly"), ml.map(x=>x.ym.slice(5)), ml.map(x=>x.net));
      const topCb = [...perCashbox].sort((a,b)=>Math.abs(b.net)-Math.abs(a.net)).slice(0,8);
      drawBarChart($("#chart-cashboxes"), topCb.map(x=> (x.name||"").slice(0,6)), topCb.map(x=>x.net));
    }catch(e){}
  })();
...
// Loading safety: if auth doesn't resolve quickly, show a friendly fallback
let authResolved = false;
setTimeout(()=>{
  if(!authResolved){
    showFatalLoadingProblem();
  }
}, 8000);

const APP_BUILD = "v6";
console.info("CashBoxes Pro build:", APP_BUILD);
